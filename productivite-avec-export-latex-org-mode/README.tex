...
\section{Contexte}
\label{sec:orgbf84d1b}

\begin{frame}[label={sec:org44de397}]{Contexte}
\begin{quote}
...
\begin{frame}[label={sec:orgf66b356}]{Agenda}
\begin{center}
...
\label{sec:org400449d}

\begin{frame}[fragile,label={sec:org012ccdf}]{Liste \og à puces\fg{} en \LaTeX{}}
 Code \LaTeX{}
...
\begin{frame}[fragile,label={sec:org05553ec}]{Liste numérotée en \LaTeX{}}
 Code \LaTeX{}
...
\begin{frame}[fragile,label={sec:org4358e87}]{Liste de description en \LaTeX{}}
 Code \LaTeX{}
...
\begin{frame}[fragile,label={sec:org64e519e}]{Tableau en \LaTeX{}}
 \begin{itemize}
...
\begin{frame}[plain,label={sec:org04575f1}]{Mais on a toujours fait comme cela\ldots{}}
...
\label{sec:org8f37b39}

\begin{frame}[plain,label={sec:org14b38eb}]{Org à la rescousse}
\setbeamertemplate{navigation symbols}{}
\begin{tikzpicture}[remember picture,overlay]
    \node[xshift=0cm,yshift=0cm] at (current page.center) {
...
\begin{frame}[label={sec:org0806932}]{Présentation}
\begin{columns}
...
\begin{frame}[fragile,label={sec:org72d1b87}]{Mise en forme}
 Balisage virtuellement inexistant

\begin{columns}
\begin{column}{0.48\columnwidth}
\lstset{language=org,label=org8366377,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
Normal
...
\begin{frame}[fragile,label={sec:orgec97adf}]{Liste \og à puces\fg{} en Org mode}
 Affichage
...
\begin{frame}[fragile,label={sec:org7405dca}]{Liste numérotée en Org mode}
 Affichage
...
\begin{frame}[fragile,label={sec:orge776eed}]{Liste de description en Org mode}
 Affichage
...
\begin{frame}[fragile,label={sec:orgd315f12}]{Tableau en Org mode}
 \begin{columns}
...
\begin{frame}[plain,label={sec:org6c3817d}]{Définition}
\setbeamertemplate{navigation symbols}{}
\begin{tikzpicture}[remember picture,overlay]
    \node[xshift=0cm,yshift=0cm] at (current page.center) {
...
\begin{frame}[label={sec:orga1d738a}]{Création de tableaux}
\begin{itemize}
...
\begin{frame}[label={sec:org2c54880}]{Édition de tableaux}
\begin{itemize}
\item Pour supprimer
\begin{itemize}
...
\begin{frame}[fragile,label={sec:orgec9e62c}]{Références}
 \begin{itemize}
...
\begin{frame}[fragile,label={sec:orgf66006d}]{Formules}
 \begin{columns}
\begin{column}{0.55\columnwidth}
Syntaxe Org

\lstset{language=org,label=org4ad9111,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\begin{frame}[fragile,label={sec:orgb32be4a}]{Formules}
 \begin{itemize}
...
\begin{frame}[label={sec:orgef344e0}]{Fonctions}
\begin{itemize}
...
\begin{frame}[label={sec:orgb5ff6d7}]{Fonctions}
\begin{itemize}
\item Statistique
\begin{center}
\begin{tabular}{ll}
...
\begin{frame}[fragile,label={sec:org7b3fb7f}]{Format}
 \begin{center}
\begin{tabular}{ll}
...
\begin{frame}[fragile,label={sec:orga4e5bed}]{Assistance à l'édition de la ligne \texttt{\#+TBLFM}}
 \begin{itemize}
...
\begin{frame}[fragile,label={sec:org014e77e}]{Tableau Org dans source \LaTeX{}}
 \begin{itemize}
...
\begin{frame}[fragile,label={sec:org6cdcabe}]{Code en R}
...
\begin{frame}[label={sec:orga6acb86}]{Graphique généré en R}
\begin{columns}
\begin{column}{0.36\columnwidth}
\begin{table}[!htbp]
\caption{\label{tab:orgd5dd468}
Achats par mois}
\centering
\begin{tabular}{lr}
Mois & Montant\\
...
\begin{frame}[label={sec:org3031bb0}]{Graphique généré en PGFPlots}
\begin{columns}
\begin{column}{0.36\columnwidth}
\begin{tikzpicture}[font={\rmfamily}]
...
\begin{frame}[fragile,label={sec:orgda8a3f1}]{Code SQL}
 \lstset{language=org,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\begin{frame}[fragile,label={sec:org8bebfb9}]{Autres environnements}
 Blocs spéciaux
...
\begin{frame}[fragile,label={sec:org5604ec9}]{Sectionnement}
...
\begin{frame}[label={sec:org3887df4}]{Édition de la structure}
\begin{itemize}
\item \alert{Section}
\begin{itemize}
...
\begin{frame}[fragile,label={sec:org12d8efd}]{Visibilité}
 \lstset{language=org,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
* Introduction...
* Expériences...
...
\begin{frame}[fragile,label={sec:orgdb6c8d2}]{Écrire un rapport}
 \lstset{language=org,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\begin{frame}[fragile,label={sec:org374154b}]{Générer un index}
 \begin{itemize}
...
\begin{frame}[fragile,label={sec:org2ffde0b}]{Écrire une présentation}
 \begin{itemize}
...
\begin{frame}[fragile,label={sec:org11d6cf1}]{Écrire un examen}
...
\begin{frame}[label={sec:org51a721a}]{Écrire une documentation logicielle}
...
\begin{frame}[fragile,label={sec:orga8874d8}]{Coloration syntaxique}
 \begin{itemize}
\item À l'export
...
\begin{frame}[label={sec:orgf0ab5bf}]{Écrire une thèse}
...
\begin{frame}[fragile,label={sec:org236e9e0}]{Exécution de blocs de code}
 \begin{itemize}
...
\begin{frame}[fragile,label={sec:org226dec5}]{Mathématiques}
...
\begin{frame}[fragile,label={sec:org093f7db}]{Écrire une lettre}
...
\begin{frame}[label={sec:orgd1b0b2c}]{Exporter le document Org}
...
\begin{frame}[label={sec:org5934feb}]{Exporter en Beamer}
\begin{columns}
\begin{column}{0.48\columnwidth}
\begin{center}
...
\begin{frame}[fragile,label={sec:org8a86fcf}]{Exports à jour}
 \lstset{language=Lisp,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\begin{frame}[label={sec:orgaeb0899}]{Suppléments non abordés}
\begin{itemize}
...
\section{Conclusion}
\label{sec:org184dc3c}

\begin{frame}[<+->][label={sec:org0d6727a}]{Cliquer ici}
...
\begin{frame}[label={sec:orgf42d329}]{Conclusion}
\begin{itemize}
...
\begin{frame}[plain,label={sec:orgaa081d9}]{Conclusion}
\setbeamertemplate{navigation symbols}{}
\begin{tikzpicture}[remember picture,overlay]
    \node[xshift=0cm,yshift=0cm] at (current page.center) {
...
\begin{frame}[label={sec:org56f4df5}]{Coordonnées}
Fabrice Niessen \\
