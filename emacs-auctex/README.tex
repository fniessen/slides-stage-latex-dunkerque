...
\section{Aperçu}
\label{sec:org552f5d3}

\begin{frame}[label={sec:orgc128369}]{Description}
...
\begin{frame}[label={sec:orgabcaef1}]{Caractéristiques et objectifs}
...
\begin{frame}[label={sec:orga70de46}]{Exigences}
\begin{itemize}
...
\begin{frame}[label={sec:org3980eac}]{Besoin de plusieurs outils dans votre boîte}
\begin{figure}[!htbp]
\centering
...
\begin{frame}[label={sec:orgd018f24}]{Emacs intègre tous ces outils}
\begin{columns}
...
\begin{frame}[label={sec:orgb8a7fe7}]{Productivité = capacité à ÉDITER DU TEXTE}
...
\label{sec:orgf090736}

\begin{frame}[label={sec:orgadfcf36}]{Terminologie Emacs}
\begin{figure}[!htbp]
\centering
...
\begin{frame}[label={sec:org476981f}]{Ne pas utiliser la souris}
\begin{figure}[!htbp]
\centering
...
\begin{frame}[label={sec:org3bf7b5a}]{Exemple de fichier \LaTeX{} sous Emacs}
\begin{figure}[!htbp]
\centering
...
\begin{frame}[label={sec:org8e2393d}]{Notation des raccourcis clavier}
\begin{figure}[!htbp]
\centering
...
\begin{frame}[label={sec:orgc89c3bb}]{Manipuler un fichier}
\begin{description}
...
\begin{frame}[label={sec:org1e2e062}]{Manipuler un écran}
...
\begin{frame}[label={sec:org7f07407}]{Annuler}
\begin{description}
...
\begin{frame}[label={sec:org0ebd8c2}]{Couper / Copier / Coller}
\begin{description}
...
\begin{frame}[fragile,label={sec:orgb28b5b7}]{Chercher}
 \begin{description}
...
\begin{frame}[label={sec:orga067b87}]{Rechercher / remplacer}
\begin{description}
...
\begin{frame}[label={sec:org6d6a5aa}]{Manipuler des lignes}
\begin{description}
...
\begin{frame}[label={sec:org804d495}]{Mettre en forme du texte}
\begin{description}
...
\begin{frame}[label={sec:orgb71e7fd}]{Manipuler un \emph{buffer}}
...
\begin{frame}[fragile,label={sec:org935e175}]{Ligne de mode}
 \begin{enumerate}
...
\begin{frame}[label={sec:orgd968960}]{Obtenir de l'aide}
\begin{description}
\item[{\keys{C-h} \keys{t}}] Accéder au tutorial d'Emacs
\end{description}
\end{frame}

\begin{frame}[fragile,label={sec:org713c4e1}]{Configurer Emacs}
 \begin{itemize}
...
\begin{frame}[label={sec:org140cc81}]{Fonctionnalités supplémentaires}
...
\begin{frame}[fragile,label={sec:org62741b0}]{Helm (levier de commande)}
 \begin{itemize}
...
\begin{frame}[fragile,label={sec:orgd36a2ea}]{Sauvegarde de la position du curseur}
 \lstset{language=Lisp,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\begin{frame}[fragile,label={sec:orgb7d914d}]{Vérification orthographique à la volée}
 \lstset{language=Lisp,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\begin{frame}[fragile,label={sec:org0f87691}]{Vérification syntaxique à la volée}
 \lstset{language=Lisp,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
(require 'flymake)
...
\begin{frame}[fragile,label={sec:orgb825f09}]{Compilation \LaTeX{} par défaut}
 \lstset{language=Lisp,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
(defun TeX-default ()
...
\label{sec:orge1e0b5d}

\subsection{AUC\TeX{}}
\label{sec:orgedb0b80}

\begin{frame}[label={sec:orgca22043}]{Description}
\begin{itemize}
...
\begin{frame}[fragile,label={sec:orgc6e2d6e}]{Appariage de certains caractères}
 \lstset{language=Lisp,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\begin{frame}[fragile,label={sec:orgfde953a}]{Mise en forme du texte}
 \begin{description}
...
\begin{frame}[fragile,label={sec:org2eaf105}]{Insertion de sections}
 \begin{description}
...
\begin{frame}[label={sec:orgcddaf17}]{Insertion de commandes}
\begin{description}
...
\begin{frame}[fragile,label={sec:org77c8f3f}]{Insertion d'environnements}
 \begin{description}
...
\begin{frame}[fragile,label={sec:org202010e}]{Compilation}
 \begin{description}
...
\begin{frame}[fragile,label={sec:orged29882}]{Erreurs}
 \begin{description}
...
\begin{frame}[label={sec:orgc8dce08}]{Documentation \TeX{}}
\begin{description}
...
\begin{frame}[fragile,label={sec:org83a01c2}]{Activation d'AUC\TeX{}}
 Installation via le
...
\begin{frame}[fragile,label={sec:orgd59bf7a}]{Quelques variables d'intérêt}
 \lstset{language=Lisp,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\begin{frame}[label={sec:org1dd7a3d}]{Formatage}
\begin{description}
...
\subsection{RefTeX}
\label{sec:org850fcb2}

\begin{frame}[fragile,label={sec:org1116b43}]{Description}
...
\begin{frame}[label={sec:org72ddf1f}]{Table des matières}
\begin{description}
...
\begin{frame}[fragile,label={sec:orgd3e144c}]{Références}
 \begin{description}
...
\begin{frame}[fragile,label={sec:orge4ab8cb}]{Références croisées}
 \begin{description}
...
\begin{frame}[fragile,label={sec:org6f4d21f}]{Citations bibliographiques}
 \begin{description}
...
\begin{frame}[label={sec:org8c87d86}]{Index}
\begin{description}
...
\begin{frame}[label={sec:org2a9c4a8}]{Promotion / « démotion » de niveau}
\begin{description}
...
\begin{frame}[fragile,label={sec:org63297eb}]{Activation de RefTeX}
 \lstset{language=Lisp,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\label{sec:orgb27dee7}

\begin{frame}[fragile,label={sec:orgbb9df5d}]{Description}
...
\label{sec:org2e68345}

\begin{frame}[fragile,label={sec:orgade84c4}]{Orgtbl}
...
\begin{frame}[c,plain,label={sec:orgcc7e848}]{}
\begin{center}
\Huge{Super présentation, \\ mais on doit terminer à temps}
\end{center}
\end{frame}

\begin{frame}[fragile,label={sec:orgf2b5a4c}]{Outline}
...
\begin{frame}[label={sec:orga5c4257}]{Édition de rectangles}
\begin{description}
...
\begin{frame}[fragile,label={sec:org1339ec7}]{Enregistrer et exécuter une macro}
 \begin{description}
...
\begin{frame}[label={sec:orgc167ce2}]{M-x ediff-buffers}
\begin{figure}[!htbp]
\centering
...
\begin{frame}[label={sec:orgdd3b02a}]{Dired}
\begin{figure}[!htbp]
\centering
...
\begin{frame}[fragile,label={sec:orgcc0d200}]{Dictionary mode}
 \lstset{language=Lisp,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\begin{frame}[fragile,label={sec:orga52e019}]{Highlight line}
 \lstset{language=Lisp,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
(require 'hl-line)
(hl-line-mode)
...
\section{Conclusions}
\label{sec:orgf6b7470}

\begin{frame}[label={sec:orgb3a6c76}]{Continuer à apprendre}
\begin{itemize}
...
\begin{frame}[label={sec:orgc37e0b0}]{Ne pas se laisser intimider}
...
\begin{frame}[fragile,label={sec:orgd48f87d}]{Coordonnées}
